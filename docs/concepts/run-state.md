...
{"event":"verify:start","ts":"...","iteration":1,"command":"bundle exec rake test"}
{"event":"verify:pass","ts":"...","iteration":1}
...
| `verify:start` | `--verify-command` started (`command`). |
| `verify:pass` / `verify:fail` | Verification verdict. |
...
| `iteration:verify_failure` | Robot claimed success but verification failed. |
| `orchestrator:abort` | Run stopped by a stop condition or permanent error (`reason`). |
